...
use glm::{Mat4, identity, vec3};
use image::{EncodableLayout, ImageReader};
use transformations::gl_utils::{
    self,
...
const TITLE: &str = "HELLO TRANSFORMATIONS!";
...
fn render_loop(
    ...
    while !window.should_close() {
        ...
        unsafe {
            ...
            transform = glm::translate(&transform, &vec3(0.5, -0.5, 0.0));
            transform = glm::rotate(
                &transform,
                f32::to_radians(glfwGetTime() as f32 * 100.),
                &glm::vec3(0., 0., 1.),
            );

            let mut transform2: Mat4 = identity();
            transform2 = glm::translate(&transform2, &vec3(-0.5, 0.5, 0.0));
            transform2 = glm::rotate(
                &transform2,
                f32::to_radians(glfwGetTime() as f32 * 500.),
                &glm::vec3(0., 0., 1.),
            );

            shader_program
                .set_uniform_mat_4fv("transform", transform)
                .unwrap_or_else(|e| panic!("{e}"));
            ...
            shader_program
                .set_uniform_mat_4fv("transform", transform2)
                .unwrap_or_else(|e| panic!("{e}"));
