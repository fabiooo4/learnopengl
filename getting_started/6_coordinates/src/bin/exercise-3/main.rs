...
fn render_loop(
    ...
    while !window.should_close() {
        ...
        unsafe {
            ...
            view_mat = glm::translate(&view_mat, &vec3(0., 0., -3.));
            ...
            for (i, pos) in cube_positions.iter().enumerate() {
                ...
                let angle = 20. * (i + 1) as f32;
                let time = if i % 3 == 0 { glfwGetTime() as f32 } else { 1. };

                model_mat = glm::rotate(&model_mat, time * f32::to_radians(angle), &vec3(1., 0.3, 0.5));
